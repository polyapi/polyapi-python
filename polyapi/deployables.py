...
from typing import Literal, TypedDict, List, Dict, Tuple, Optional, Any, Union
...
CACHE_VERSION_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "deployments_revision"
)
CACHE_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "cached_deployables"
)


DeployableTypeNames = Literal["PolyServerFunction", "PolyClientFunction"]
DeployableTypes = Literal["server-function", "client-function"]
...
DeployableTypeToName: Dict[DeployableTypeNames, DeployableTypes] = {
    name: type for name, type in DeployableTypeEntries
}
...
def load_deployable_records() -> List[DeployableRecord]:
    return [
        read_json_file(os.path.join(CACHE_DIR, name))
...
def save_deployable_records(records: List[DeployableRecord]) -> None:
    for record in records:
        write_json_file(
            os.path.join(CACHE_DIR, f'{record["context"]}.{record["name"]}.json'),
            record,
        )
...
def get_all_deployable_files_windows(config: PolyDeployConfig) -> List[str]:
    ...
    include_pattern = (
        " ".join(
            f"*.{f}" if "." in f else f"*.{f}"
            for f in config["include_files_or_extensions"]
        )
        or "*"
    )
    exclude_pattern = "|".join(config["exclude_dirs"])
    pattern = (
        "|".join(f"polyConfig: {name}" for name in config["type_names"]) or "polyConfig"
    )
...
def get_all_deployable_files_linux(config: PolyDeployConfig) -> List[str]:
    ...
    patterns = (
        " ".join(f"-e 'polyConfig: {name}'" for name in config["type_names"])
        or "-e 'polyConfig'"
    )
...
def get_all_deployable_files(config: PolyDeployConfig) -> List[str]:
    ...
    if not config.get("type_names"):
        config["type_names"] = [
            entry[0] for entry in DeployableTypeEntries
        ]  # Assuming DeployableTypeEntries is defined elsewhere
...
def get_git_revision(branch_or_tag: str = "HEAD") -> str:
    try:
        return check_output(
            ["git", "rev-parse", "--short", branch_or_tag], text=True
        ).strip()
...
def write_deploy_comments(deployments: List[Dict]) -> str:
    ...
    canopy_path = (
        "polyui/collections"
        if "localhost" in os.getenv("POLY_API_BASE_URL", "")
        else "canopy/polyui/collections"
    )
    comments = []
    for d in deployments:
        instance_url = (
            d["instance"].replace(":8000", ":3000")
            if d["instance"].endswith(":8000")
            else d["instance"]
        )
...
def print_docstring_function_comment(
    description: str, args: list, returns: dict
) -> str:
...
def update_deployable_function_comments(
    file_content: str, deployable: dict, disable_docs: bool = False
) -> str:
    """
...
def write_updated_deployable(deployable: dict, disable_docs: bool = False) -> dict:
    ...
    if deployable["type"] in ["client-function", "server-function"]:
        file_contents = update_deployable_function_comments(
            file_contents, deployable, disable_docs
        )
    else:
...
def write_deploy_comments(deployments: list) -> str:
    ...
    for d in deployments:
        ...
        comments.append(
            f"# Poly deployed @ {d['deployed']} - {d['context']}.{d['name']} - {instance_url}/{canopy_path}/{d['type']}s/{d['id']} - {d['fileRevision']}"
        )
    return "\n".join(comments)
