...
def _abandon_close_task(
    ...
    The task is cancelled and dropped rather than awaited: waiting for an
    unresponsive pool to unwind is the stall the deadline exists to prevent.
    Cancelling is what keeps loop shutdown bounded too, since asyncio.run()
    tears down by cancelling every task and gathering them; a cancelled close
    completes there instead of blocking. Whatever stays open is released when
    the coroutine is collected, and by the OS at exit.
    """
